            ...
            // Initialize backend URL immediately for web mode
            (function initBackendUrl() {
                try {
                    // Check if already set
                    if (window.__AlphaOps_BACKEND_URL) {
                        console.log('[init] Backend URL already set:', window.__AlphaOps_BACKEND_URL);
                        return;
                    }

                    // Try localStorage first
                    const storedUrl = localStorage.getItem('AlphaOps_backend_url');
                    if (storedUrl) {
                        window.__AlphaOps_BACKEND_URL = storedUrl;
                        console.log('[init] Using backend URL from localStorage:', storedUrl);
                        return;
                    }

                    // Default to production backend
                    window.__AlphaOps_BACKEND_URL = 'https://AlphaOps-global-20260203.onrender.com';
                    localStorage.setItem('AlphaOps_backend_url', window.__AlphaOps_BACKEND_URL);
                    console.log('[init] Using default backend URL:', window.__AlphaOps_BACKEND_URL);
                } catch (e) {
                    console.error('[init] Failed to set backend URL:', e);
                    // Fallback
                    window.__AlphaOps_BACKEND_URL = 'https://AlphaOps-global-20260203.onrender.com';
                }
            })();
            ...
            const POLLING_INTERVAL_KEY = 'AlphaOps-monitoring-refresh-interval';
            ...
            async function pushMonitoringConfigToIframe() {
                ...
                let token = window.__AlphaOps_ACCESS_TOKEN;
                if (!token) {
                    try {
                ...
                const backendUrl = (window.__AlphaOps_BACKEND_URL || '').toString().replace(/\/+$/, '');
                    ...
                    try {
                        ...
                        if (status?.success && status.online) {
                            ...
                            if (result.success) {
                                ...
                                // Reload servers from database to get the newly connected agent
        ...
        `;
            ...
            let currentUser = { name: 'Admin', email: 'admin@AlphaOps.io', avatar: 'https://ui-avatars.com/api/?name=Admin&background=random' };
            ...
            const views = {
                ...
                <div class="flex flex-col gap-6 h-full">
                    ...
                    <div class="flex flex-col lg:flex-row gap-8 items-stretch flex-1 min-h-0">
                        ...
                        <div class="premium-dashboard-card rounded-[2.5rem] flex flex-col w-full lg:w-[70%] shadow-2xl shadow-slate-200/40 relative overflow-hidden bg-white border border-slate-100">
            ...
            `,
                ...
                <div id="cicd-builder-container" class="hidden fixed inset-0 z-[100] bg-slate-50">
                    <div id="AlphaOps-pipeline-root" class="w-full h-full"></div>
                </div>
            `,
                ...
                <div class="flex flex-col h-[calc(100vh-160px)] bg-black rounded-xl shadow-2xl overflow-hidden border border-gray-800">
                    ...
                    <div class="flex-1 p-4 font-mono text-sm relative overflow-hidden flex flex-col bg-black" onclick="document.getElementById('terminal-input').focus()">
                        ...
                        <div id="terminal-output" class="flex-1 overflow-y-auto scrollbar-hide" style="font-family: 'Consolas', 'Menlo', monospace;">
                            ...
                            <div class="text-gray-400 mb-2">AlphaOps SSH Client v2.0 - Authenticated</div>
            ...
            function getPipelineApiBase() {
                ...
                const base = (window.__AlphaOps_BACKEND_URL || '').toString().replace(/\/+$/, '');
            ...
            async function apiFetch(path, init = {}) {
                ...
                const token = window.__AlphaOps_ACCESS_TOKEN;

                const baseUrl = getPipelineApiBase();

                if (!baseUrl) {

                    setPipelineBackendBanner('Backend disconnected. Set window.__AlphaOps_BACKEND_URL.');
            ...
            window.openCreatePipelineView = function () {
                ...
                if (mainC && buildC) {
                    ...
                    if (window.AlphaOpsPipelineMount) {
                        window.AlphaOpsPipelineMount();
                    }
                } else {
            ...
            window.generateYamlPreview = function () {
                ...
                let yaml = `# AlphaOps WORKFLOW GENERATED PREVIEW\n`;
            ...
            async function runPipeline(id) {
                ...
                try {
                    ...
                    if (window.AlphaOpsPipelineOpenRun && definition) {

                        window.AlphaOpsPipelineOpenRun({ runId: started.runId, pipelineId: id, definition });
            ...
            function navigate(viewName, params = null) {
                ...
                if (viewName === 'servers' || viewName === 'all-servers' || viewName === 'all-pipelines' || viewName === 'cicd') {
                    ...
                    // Auto-open DevAI panel on servers view with no close button
                    const devaiPanel = document.getElementById('devai-inline-panel');
                    const devaiCloseBtn = document.getElementById('devai-close-button');
                    if (devaiPanel && !devaiPanel.classList.contains('devai-open')) {
                        // Open DevAI panel for servers view
                        devaiPanel.classList.add('devai-open');
                        devaiPanel.style.setProperty('max-width', '30%', 'important');
                        devaiPanel.style.setProperty('width', '30%', 'important');
                        devaiPanel.style.setProperty('min-width', '320px', 'important');
                        devaiPanel.classList.remove('w-0');
                        devaiPanel.classList.add('border-l');
                        devaiPanel.classList.remove('border-l-0');
                    }
                    // Hide close button on servers view
                    if (devaiCloseBtn) {
                        devaiCloseBtn.classList.remove('devai-close-visible');
                    }
                }
                // SERVER SPECIFIC VIEWS
                    ...
                    // Show close button and close DevAI by default on server-specific views
                    const devaiPanel = document.getElementById('devai-inline-panel');
                    const devaiCloseBtn = document.getElementById('devai-close-button');
                    if (devaiPanel && devaiPanel.classList.contains('devai-open')) {
                        // Close DevAI panel when entering a server-specific view
                        devaiPanel.classList.remove('devai-open');
                        devaiPanel.style.setProperty('max-width', '0', 'important');
                        devaiPanel.style.setProperty('width', '0', 'important');
                        devaiPanel.classList.add('w-0');
                        devaiPanel.classList.remove('border-l');
                        devaiPanel.classList.add('border-l-0');
                    }
                    // Show close button when connected to a server
                    if (devaiCloseBtn) {
                        devaiCloseBtn.classList.add('devai-close-visible');
                    }
                }
                ...
                if (viewWrapper) {
                    ...
                    if (viewName === 'files' || viewName === 'terminal') {
                        ...
                        if (viewName === 'cicd') {
                            ...
                            setTimeout(() => {
                                if (window.AlphaOpsPipelineMount) window.AlphaOpsPipelineMount();
                            }, 50);
                        } else {
                            viewWrapper.innerHTML = views[viewName]();
                        }
                    ...
                    if (viewName === 'cicd') {
                        ...
                        setTimeout(() => {
                            if (window.AlphaOpsPipelineMount) {
                                ...
                                console.error('AlphaOpsPipelineMount not found. Pipeline app may not be loaded.');
                            }
                        }, 50);
                    }
            ...
            async function connectToServer(e) {
                ...
                // Fields
                const name = document.getElementById('server-name').value.trim() || (connectionMode === 'agent' ? "AlphaOps Agent" : "SSH Server");
                ...
                try {
                    ...
                    if (connectionMode === 'agent') {
                        const server = window.editingServerId
                            ? userServers.find(s => s.id === window.editingServerId)
                            : null;
                        const agentId = String(server?.host || '').replace(/^agent:/, '').trim();
                        if (!agentId) throw new Error('AgentId is missing. Please enroll an agent and select it from Servers list.');
                        res = await ipcRenderer.invoke('agent:connect', { agentId });
                    } else {
                    ...
                    if (res && res.success) {
                        const serverId = (connectionMode === 'agent')
                            ? (String(res.serverId || res.id || '').trim() || String(res.agentId || '').trim() || String(userServers.find(s => s.id === window.editingServerId)?.host || '').replace(/^agent:/, '').trim())
                            : '';
                        await onConnectionSuccess({
                            host: (connectionMode === 'agent') ? `agent:${String(userServers.find(s => s.id === window.editingServerId)?.host || '').replace(/^agent:/, '').trim()}` : host,
                            username: (connectionMode === 'agent') ? 'agent' : username,
                            keyPath: keyPath || '',
                            id: (connectionMode === 'agent') ? serverId : Date.now().toString(),
                            serverName: name,
                            shouldSaveToCloud,
                            isElastic,
                            mode: connectionMode
            ...
            async function onConnectionSuccess(details) {
                ...
                window.connectedServerData = connectedServerData;
                currentFilesPath = '~';
            ...
            const PROFILE_CACHE_KEY = 'AlphaOps_profile_cache';
            ...
            async function initProfile() {
                ...
                try {
                    ...
                    try {
                        ...
                        window.__AlphaOps_ACCESS_TOKEN = session.access_token;
                    ...
                    try {
                        ...
                        // Fallback for web mode - check localStorage or use default
                        const storedUrl = localStorage.getItem('AlphaOps_backend_url');
                        if (storedUrl) {
                            window.__AlphaOps_BACKEND_URL = storedUrl;
                            console.log('[init] Using backend URL from localStorage:', storedUrl);
                    ...
                    localStorage.removeItem('AlphaOps_servers');
            ...
            async function logoutApp() {
                ...
                if (confirm("Sign out of AlphaOps?")) {
            ...
            function openSettingsModal() {
                ...
                const savedDownloadPath = localStorage.getItem('AlphaOps:downloadPath') || '';
            ...
            async function changeDownloadFolder() {
                ...
                try {
                    ...
                    if (res && res.success && res.path) {
                        ...
                        localStorage.setItem('AlphaOps:downloadPath', res.path);
            ...
            async function disconnectServer() {
                ...
                window.connectedServerData = null;
            ...
            async function downloadSelectedFiles() {
                ...
                const localDir = localStorage.getItem('AlphaOps:downloadPath') || '';
