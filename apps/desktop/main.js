...
const { ApiClient } = require('@devyntra/api-client');
...
function getDesktopBackendBaseUrl() {
    ...
    const envUrl = process.env.DEVYNTRA_BACKEND_URL || process.env.BACKEND_URL;
    ...
    // Fallbacks (remove or override via DEVYNTRA_BACKEND_URL env)

    return 'https://devyntra-global-20260203.onrender.com';
}
...
const sshClient = {
    ...
    async connect(config) {
        ...
        if (host.startsWith('agent:')) {
            ...
            apiClient.setBaseUrl('https://devyntra-backend-api-production.up.railway.app');
            apiClient.setToken(currentAccessToken);
            const result = await apiClient.connectAgent(agentId);
            this.isConnected = true;
            this.connectionType = 'agent';
            this.baseUrl = 'https://devyntra-backend-api-production.up.railway.app';
            return result;
        }
...
sshClient.exec = async (command) => {
    ...
    const isStats = (
        ...
        normalizedCommand.includes('> ~/.devyntra/apps.json') // Internal DB save
...
if (!gotTheLock) {
    ...
    app.on('second-instance', (event, commandLine, workingDirectory) => {
        ...
        // commandLine looks like: [path_to_exe, "devyntra://..."]
...
function handleDeepLink(url) {
    ...
    // Structure: devyntra://auth/callback#access_token=...&refresh_token=...

    // Or: devyntra://auth/callback?code=... (PKCE)
    ...
    res.end(`
        ...
        <body>
            ...
            <script>
                ...
                const appUrl = 'devyntra://auth/callback' + search + hash;
...
ipcMain.handle('ssh:connect', async (event, config) => {
    ...
    try {
        ...
        // This fixes "App stops working when I close Devyntra"
...
ipcMain.handle('ssh:download-items', async (event, payload) => {
    ...
    try {
        ...
        const tmpName = `devyntra-download-${Date.now()}.tar.gz`;
...
`;
            ...
            const startScriptPath = `${fullPath}/devyntra_start_none.sh`;
            ...
            await sshClient.exec(`bash -lc 'test -f ~/.devyntra/${name}.pid && kill -9 $(cat ~/.devyntra/${name}.pid) >/dev/null 2>&1 || true'`).catch(() => { });

            const logPath = `~/.devyntra/${name}.nohup.log`;
            const pidRes = await sshClient.exec(`bash -lc 'mkdir -p ~/.devyntra; nohup "${startScriptPath}" > ${logPath} 2>&1 & echo $!'`);
            const bgPid = String(pidRes.stdout || '').trim();
            if (!bgPid) throw new Error('Failed to start app in background (no PID returned).');
            await sshClient.exec(`bash -lc 'echo "${bgPid}" > ~/.devyntra/${name}.pid'`);
    ...
    location / {
        ...
        const registryCmd = `mkdir -p ~/.devyntra && touch ~/.devyntra/apps.json && cat ~/.devyntra/apps.json`;
        ...
        await sshClient.exec(`echo "${b64}" | base64 -d > ~/.devyntra/apps.json`);
...
ipcMain.handle('ssh:list-apps', async () => {
    ...
    try {
        ...
        const res = await sshClient.exec('cat ~/.devyntra/apps.json');
        ...
        for (const app of apps) {
            ...
            try {
                ...
                if (app.manager === 'systemd') {
                    ...
                    if (!pid) {
                        try {
                            const pidRes = await sshClient.exec(`bash -lc 'test -f ~/.devyntra/${app.id}.pid && cat ~/.devyntra/${app.id}.pid || true'`);
...
ipcMain.handle('ssh:manage-app', async (event, { id, action }) => {
    ...
    try {
        ...
        const regRes = await sshClient.exec('cat ~/.devyntra/apps.json');
        ...
        if (app.manager === 'systemd') {
            ...
            const pidFile = `~/.devyntra/${id}.pid`;
            const logFile = `~/.devyntra/${id}.nohup.log`;
            const startScriptPath = (app.path ? `${app.path}/devyntra_start_none.sh` : `~/apps/${id}/devyntra_start_none.sh`);
            ...
            if (action === 'stop') {
                ...
                await killPid();
                const pRes = await sshClient.exec(`bash -lc 'mkdir -p ~/.devyntra; nohup "${startScriptPath}" > ${logFile} 2>&1 & echo $!'`);
        ...
        await run(`echo "${b64}" | base64 -d > ~/.devyntra/apps.json`);
...
ipcMain.handle('ssh:add-cron', async (event, { name, schedule, command, stopDateTime }) => {
    ...
    try {
        ...
        if (stopDateTime) {
            ...
            const removalScript = `~/.devyntra/remove_cron_${cleanName}.sh`;
...
crontab -l 2>/dev/null | grep -v "# DEVYNTRA_STOP_${cleanName}" | crontab -
...
chmod +x ${removalScript}
            ...
            const removalCronLine = `${minute} ${hour} ${day} ${month} * ${removalScript} # DEVYNTRA_STOP_${cleanName}`;
...
ipcMain.handle('ssh:save-script', async (event, { filename, content }) => {
    ...
    try {
        ...
        await sshClient.exec('mkdir -p ~/.devyntra/scripts');
        ...
        const saveCmd = `echo "${b64}" | base64 -d > ~/.devyntra/scripts/${filename}`;
        ...
        res = await sshClient.exec(`chmod +x ~/.devyntra/scripts/${filename}`);



        // Return full path

        const pathRes = await sshClient.exec(`echo ~/.devyntra/scripts/${filename}`);
...
ipcMain.handle('cicd:run-pipeline', async (event, { pipeline, serverConfig }) => {
    ...
    try {
        ...
        const context = {
            ...
            WORK_DIR: `/tmp/devyntra-builds/${pipeline.name.replace(/\s+/g, '-')}-${Date.now()}`
...
// Ensure .devyntra/keys exists

const DEVYNTRA_DIR = path.join(os.homedir(), '.devyntra');

const KEYS_DIR = path.join(DEVYNTRA_DIR, 'keys');



if (!fs.existsSync(DEVYNTRA_DIR)) {

    try { fs.mkdirSync(DEVYNTRA_DIR); } catch (e) { console.error("Failed to create .devyntra dir", e); }
