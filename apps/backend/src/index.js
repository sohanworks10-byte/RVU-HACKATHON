...
app.post('/ssh/deploy', requireUser, async (req, res) => {
  try {
    ...
    const readRegistry = async () => {
      const readRes = await execNoThrow('mkdir -p ~/.AlphaOps && touch ~/.AlphaOps/apps.json && cat ~/.AlphaOps/apps.json');
    ...
    const writeRegistry = async (apps) => {
      ...
      await execOrThrow(`echo '${escaped}' > ~/.AlphaOps/apps.json`);
    };
    ...
    const writeSudoFile = async (targetPath, content) => {
      const tmpPath = `/tmp/AlphaOps_${Date.now()}_${Math.random().toString(16).slice(2)}.tmp`;
...
app.post('/agent/uninstall', requireUser, async (req, res) => {
  try {
    ...
    const uninstallCmd = `if command -v systemctl >/dev/null 2>&1; then sudo systemctl stop AlphaOps-agent || true; sudo systemctl disable AlphaOps-agent || true; sudo rm -f /etc/systemd/system/AlphaOps-agent.service || true; sudo systemctl daemon-reload || true; fi; sudo pkill -f AlphaOps-agent || true; sudo rm -rf /opt/AlphaOps-agent || true; sudo rm -f /var/log/AlphaOps-agent.log || true`;
...
// Agent command execution endpoint for web app
app.post('/agent/exec', requireUser, async (req, res) => {
  try {
    const { serverId, command } = req.body || {};
    ...
    // Execute command via agent connection
    ...
    console.error('[agent/exec] Error:', error);
...
// Get agent stats endpoint for web app
app.post('/agent/stats', requireUser, async (req, res) => {
  try {
    const { serverId } = req.body || {};
    ...
    for (const [key, cmd] of Object.entries(commands)) {
      try {
        const result = await agentConnection.exec(serverId, cmd);
...
const token = getArg('--token') || process.env.AlphaOps_AGENT_TOKEN;
const backend = getArg('--backend') || process.env.AlphaOps_BACKEND_URL;
...
console.log('[AlphaOps-agent] starting', { backend, hostname: os.hostname() });
...
function connect() {
  ...
  on('open', () => {
    ...
    console.log('[AlphaOps-agent] connected');
  ...
  on('close', () => {
    console.log('[AlphaOps-agent] disconnected; reconnecting');
    setTimeout(connect, reconnectDelay);
    reconnectDelay = Math.min(reconnectDelay * 2, 15000);
  });

  on('error', (err) => {
    console.log('[AlphaOps-agent] socket error');
  });
}
...
LOG_DIR="/var/log/AlphaOps-agent"
...
ui_line "Starting AlphaOps Agent installer..."
...
if [[ -z "$BACKEND_URL" ]]; then
  BACKEND_URL="https://AlphaOps-global-20260203.onrender.com"
fi
...
if [ -n "$BIN_URL" ]; then
  ...
  INSTALL_BIN="/usr/local/bin/AlphaOps-agent"
  set +e
  ...
  run_step "Writing agent configuration" bash -c "mkdir -p /etc/AlphaOps-agent && cat > /etc/AlphaOps-agent/config.env <<EOF
...
[Unit]
Description=AlphaOps Agent
After=network.target

[Service]
Type=simple
EnvironmentFile=/etc/AlphaOps-agent/config.env
ExecStart=/usr/local/bin/AlphaOps-agent
Restart=always
RestartSec=5
StandardOutput=append:/var/log/AlphaOps-agent/agent.log
StandardError=append:/var/log/AlphaOps-agent/agent.log

[Install]
WantedBy=multi-user.target
EOF
"
    run_step "Enabling + starting service" bash -c "systemctl daemon-reload && systemctl enable AlphaOps-agent >/dev/null 2>&1 && systemctl restart AlphaOps-agent >/dev/null 2>&1"
    ui_line "AlphaOps Agent installed and running."
    ui_line "  Service: AlphaOps-agent"
    ui_line "  Status: systemctl status AlphaOps-agent"
    ui_line "  Logs: journalctl -u AlphaOps-agent -f"
    ui_line "Install log: $LOG_FILE"
    exit 0
  fi

  run_step "Starting agent (nohup)" bash -c "nohup /usr/local/bin/AlphaOps-agent > /var/log/AlphaOps-agent/agent.log 2>&1 &"
  ui_line "AlphaOps Agent started in background (nohup)."
  ui_line "Install log: $LOG_FILE"
  exit 0
fi
...
run_step "Starting agent (nohup)" bash -c "nohup node \"$INSTALL_DIR/agent.js\" --token \"$TOKEN\" --backend \"$BACKEND_URL\" > /var/log/AlphaOps-agent/agent.log 2>&1 &"
ui_line "AlphaOps Agent started in background (nohup)."
ui_line "Install log: $LOG_FILE"
fi
`);
...
wss.on('connection', async (ws, req) => {
  try {
    ...
    // Save/update agent in database
    try {
      await query(
        `INSERT INTO agents (agent_id, user_id, host, status, last_seen, updated_at)
         VALUES ($1, $2, $3, 'online', now(), now())
         ON CONFLICT (agent_id)
         DO UPDATE SET status = 'online', host = EXCLUDED.host, last_seen = now(), updated_at = now()`,
        [record.agentId, record.userId, `agent:${record.agentId}`]
      );
      console.log(`[agent-ws] Agent ${record.agentId} saved to database`);
    } catch (dbErr) {
      console.error('[agent-ws] Failed to save agent to database:', dbErr);
    }

    ws.on('message', async (data) => {
      agentConnection.handleAgentMessage(record.agentId, data);
      
      // Update agent metadata when hello message is received
      try {
        const msg = JSON.parse(data);
        if (msg.type === 'hello') {
          await query(
            `UPDATE agents 
             SET hostname = $1, platform = $2, arch = $3, 
                 name = COALESCE(name, $1), 
                 last_seen = now(), updated_at = now()
             WHERE agent_id = $4`,
            [msg.hostname, msg.platform, msg.arch, record.agentId]
          );
          console.log(`[agent-ws] Agent ${record.agentId} metadata updated`);
        }
      } catch (e) {
    ...
    ws.on('close', async () => {
      ...
      try {
        await query(
          `UPDATE agents SET status = 'offline', updated_at = now() WHERE agent_id = $1`,
          [record.agentId]
        );
        console.log(`[agent-ws] Agent ${record.agentId} marked offline`);
      } catch (dbErr) {
        console.error('[agent-ws] Failed to update agent status:', dbErr);
      }
    });
