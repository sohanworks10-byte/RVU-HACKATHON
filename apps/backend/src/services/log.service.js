...
function maskSecrets(input) {
  ...
  if (process.env.DEVYNTRA_LOG_MASK_REGEX) {
    try {
      patterns.push(new RegExp(process.env.DEVYNTRA_LOG_MASK_REGEX, 'gi'));
    } catch {}
  }

  if (process.env.DEVYNTRA_SECRET_MASK && String(process.env.DEVYNTRA_SECRET_MASK).trim()) {
    const secrets = String(process.env.DEVYNTRA_SECRET_MASK)
      .split(',')
      .map((s) => s.trim())
      .filter(Boolean);
