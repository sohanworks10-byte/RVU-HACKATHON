...
export async function putSecret(projectId, name, value, { provider = 'auto', vaultPath } = {}) {
  ...
  if (useVault) {
    const p = vaultPath || `/v1/secret/data/AlphaOps/${encodeURIComponent(String(projectId || 'global'))}/${encodeURIComponent(String(name))}`;
...
export async function deleteSecret(projectId, name, { provider = 'auto', vaultPath } = {}) {
  const useVault = provider === 'vault' || (provider === 'auto' && isVaultConfigured());
  if (useVault) {
    const p = vaultPath || `/v1/secret/metadata/AlphaOps/${encodeURIComponent(String(projectId || 'global'))}/${encodeURIComponent(String(name))}`;
