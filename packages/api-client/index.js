    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      try {
        ...
        const looksLikeHtml =
          ...
          (payload.trim().toLowerCase().startsWith('<!doctype html') ||
            payload.trim().toLowerCase().startsWith('<html') ||
            payload.includes('<title>Error</title>'));
        ...
        throw new Error(retryableError ? 'Backend is temporarily unavailable. Please try again.' : msg);
      } finally {
